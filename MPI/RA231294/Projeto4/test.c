...
void imprime_matriz(double args[][], int n)
{
    for(int i = 0; i < n; i++)
    {
        for(int j = 0; j < n; j++)
...
int main(int argc, char** argv)
{
  ...
  if (rank==0)  // root/coordinator process
	...
	MPI_Waitany(size,ireq,&j,&stat); // join on sends
      ...
      for(i=1;i<size;i++)
	MPI_Irecv(AB+(i*interval),interval*N,MPI_DOUBLE,i,i, MPI_COMM_WORLD,ireq+i);
      for(i=1;i<size;i++)
      {
  ...
  else          // worker process
    {
      ...
      MPI_Send(AB+(rank*interval),interval*N,MPI_DOUBLE,0,rank, MPI_COMM_WORLD);
    }
  MPI_Finalize();
  imprime_matriz(AB,N);
} //main
